...
export function ChatSidebar({ rooms, roomId, user, busy, error, retry, onSelect, onNew, onProfile, onPerson }: {
  ...
  return <aside className="chat-sidebar" aria-label="Conversations">
    ...
    <div className="room-list" data-native-scroll>
      ...
      {query.trim() && <div className="sidebar-search-results"><span className="eyebrow">FIND PEOPLE</span>{search.busy ? <p className="empty-hint">Searching...</p> : search.results.map(person => <button className="person-result" key={person.id} onClick={() => onPerson(person)}><Avatar name={displayName(person)} id={person.id} online={person.online} /><span><strong>{displayName(person)}</strong><small>@{person.username}</small></span><Plus size={16} /></button>)}{!search.busy && !search.results.length && <p className="empty-hint">No users found by that username.</p>}{search.error && <p className="inline-error" role="alert">{search.error}</p>}</div>}
    </div>
